...
# that it is drawn in the centre of the vertical axis, hence half the square length must be subtracted.



   while check_if_centre(canvas, rect):
       canvas.move(rect, 1, 0)
       canvas.update()
       time.sleep(0.0166667)
   canvas.mainloop()
# This section moves the rectangle by 1 each frame until it reaches the centre of the canvas, then canvas.mainloop()
# was added to prevent the canvas from closing once the rectangle reaches the centre.


def check_if_centre(canvas, object):
    current_x = get_left_x(canvas, object)
    max_x = (CANVAS_WIDTH / 2) - (SQUARE_SIZE / 2)
    return current_x <= max_x
# check_if_center() checks whether the square is in the centre by taking the current x-coordinate(top-left) and checking
